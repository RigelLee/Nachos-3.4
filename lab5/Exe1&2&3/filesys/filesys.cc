...
void setPathFile(char *path, char *name, FileHeader *directoryHdr);
    ...
    if (directory->Find(name) != -1)
    	...
    	if (sector == -1) 		
            ...
            if (!hdr->Allocate(freeMap, initialSize, NormalFile))
                ...
                FileHeader *directoryHdr = new FileHeader;
                directoryHdr->FetchFrom(DirectorySector);
                hdr->setPath(pathFileSector, directoryHdr->getPathLength() + strlen(name));
                ...
                char *path;
                path = new char[directoryHdr->getPathLength() + strlen(name)];
                setPathFile(path, name, directoryHdr);
                pathFile->WriteAt(path, directoryHdr->getPathLength() + strlen(name), 0);
                delete pathFile;
                delete directoryHdr;
            }
            delete hdr;
...
{ 
    ...
    if (fileHdr->getFileType() == NormalFile || fileHdr->getFileType() == DirectoryFile) {
...
void setPathFile(char *path, char *name, FileHeader *directoryHdr)
{
    char *parentDirectoryPath;
    parentDirectoryPath = new char[directoryHdr->getPathLength()];
    directoryHdr->getPath(parentDirectoryPath);
    strcpy(path, parentDirectoryPath);
    strcat(path, name);
}